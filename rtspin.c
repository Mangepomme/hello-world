...
#define OPTSTR "p:c:wlveo:s:m:q:r:X:L:Q:iRu:U:Bhd:C:S::O::TD:E:A:a:H:z:"
...
int main(int argc, char** argv)
{
	...
	int matched;
	lt_t hi_exec_cost = 0;
	unsigned int hi_res_id = 0;

	int overrun_job = INT_MAX;
	...
	while ((opt = getopt(argc, argv, OPTSTR)) != -1) {
		...
		case 'a':
			...
			break;
		case 'H':
			matched = sscanf(optarg, "%llu,%u", &hi_exec_cost, &hi_res_id);
			if (matched != 2)
				usage("-H : need two arguments : WCET and reservation id");
			break;
		case 'z':
			overrun_job = want_non_negative_int(optarg, "-z");
			break;
		case ':':
	...
	param.hi_exec_cost = hi_exec_cost;
	param.hi_res_id = hi_res_id;
